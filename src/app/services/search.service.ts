 ...
 */
    ...
    getMoviesByKeyword(
        ...
        const url = `${
            this.yts_url
        }list_movies.json?query_term=${keyword}&sort_by=${sort_by}&limit=${limit}`
        return this.http.get<MoviesInt[]>(url).pipe(
            map(res => res['data']),
            catchError(this.handleError), // then handle the error
            retry(6) // retry a failed request up to 3 times
    ...
    getRandomMovies(id): Observable<MoviesInt[]> {
        const url = `https://yts.am/api/v2/movie_suggestions.json?movie_id=${id}`
        return this.http.get<MoviesInt[]>(url).pipe(
            map(res => res['data']),
            catchError(this.handleError), // then handle the error
            retry(6) // retry a failed request up to 3 times
    ...
    getShowsList(showPage): Observable<MoviesInt[]> {
        const url = `${this.base_url}/shows/${showPage}
        ?sort=trending&order=-1&genre=all`
        return this.http.get<MoviesInt[]>(url).pipe(
            retry(6), // retry a failed request up to 3 times
            catchError(this.handleError) // then handle the error
        )
    ...
    getShowsByKeyword(keyword: string): Observable<MoviesInt[]> {
        const url = `${
            this.base_url
        }/shows/1?sort=trending&order=-1&genre=all&keywords=${keyword}`
        return this.http.get<MoviesInt[]>(url).pipe(
            retry(6), // retry a failed request up to 3 times
            catchError(this.handleError) // then handle the error
        )
    ...
    getRandomShows() {
        ...
        const url = `${this.base_url}/random/show`
        return this.http.get<MoviesInt[]>(url, { headers }).pipe(
            retry(6), // retry a failed request up to 3 times
            catchError(this.handleError) // then handle the error
        )
    }
